...
class Sales extends Observable{
	...
	int[] values = { 1,2,3,4,5 };   // initial data

	void generate() {
		for (int i = 0; i < 4; i++) { // iterate once for each month
			
			int temp = values[0];
			for (int j = 0; j < 5; j++)
				values[j] = values[(j + 1) % 5];
			values[4] = temp;
			System.out.println("Printing the main" + values[4]);
...
class Table extends JFrame implements Observer{
 ...
 // The update operation is missing some code
  ...
  public void update(Observable obj, Object arg) {
	  s = (Sales) obj;
      for (int i=0; i<5; i++) {
      	table.setValueAt(s.values[i], i, 1);
