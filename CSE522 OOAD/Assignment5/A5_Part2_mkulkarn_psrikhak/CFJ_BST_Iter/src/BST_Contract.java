...
class BST_Iterator {
	...
	@Ensures({
		"!stack.isEmpty()",
		"stack.lastElement().value == root.min()"
	})
	...
	@Ensures({
		"result >= old(value)",
		//"old(stack).isEmpty() || value == old(stack).peek().value"
	})
	...
	@Ensures({
		"stack.isEmpty() || value <=  stack.peek().value",
		"old(node) == null || old(node).min() == stack.peek().value",
	})
