	...
	public TreeGUI() {
				...
				if (is_new_tree) {
					...
					try {
						if (tree == null) {
							...
							b1 = tree.insert(Integer.parseInt(s));
							if(b1){
								tm.set_state(oldTree);
							}
		...
		deleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				AbsTree tree2 = tree.clone();
				if(tree == tree.find(n) && tree.left == null && tree.right == null) {
					tree = null;
					tm.set_state(tree2);
				}
				else{
		...
		undoButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				AbsTree tree1 = tm.get_state();
				
				 if (tree1 != null) {
					 outputPanel.drawTree(tree1);
					 input_elem_text2.selectAll();
					 tree = tree1;
					 
				 }else if(is_new_tree) {
					JOptionPane.showMessageDialog(null, "Unable to undo.Tree is empty" );
				 }
